...
create database ex1_order_processing;

show databases;

use ex1_order_processing;
...
UPDATE CUST_ORDER
SET ORD_AMT = ORD_AMT * 1.10
WHERE CUSTOMERNO = 4001;
...
/*
CREATE DEFINER=`root`@`localhost` TRIGGER `cust_order_BEFORE_INSERT` BEFORE INSERT ON `cust_order` FOR EACH ROW BEGIN
